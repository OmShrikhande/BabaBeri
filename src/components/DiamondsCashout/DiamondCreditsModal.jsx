...
import { CREDIT_STATUS_OPTIONS, PAYMENT_METHOD_OPTIONS } from './constants';

const DiamondCreditsModal = ({
  isOpen,
  isEditing,
  creditForm,
  submittingCredit,
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      <div className={`absolute inset-0 bg-black/60 transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0'}`} onClick={onClose} />
      <div className={`relative bg-[#1A1A1A] border border-gray-700 rounded-xl shadow-2xl w-full max-w-2xl mx-4 transform transition-all duration-200 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8 pointer-events-none'}`}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
          <div>
            <h3 className="text-lg font-semibold text-white">
              {isEditing ? 'Edit Diamond Credit' : 'Add Diamond Credit'}
            </h3>
        ...
        <form onSubmit={onSubmit} className="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto coin-scroll">
          ...
          <div className="flex items-center justify-end space-x-3">
            ...
            >
              ...
              {submittingCredit ? 'Saving...' : isEditing ? 'Update Credit' : 'Add Credit'}
            </button>
          </div>
        </form>
      </div>
...
export default DiamondCreditsModal;
