...
const MoveAgencyForm = ({ onMoved, disabled = false }) => {
  ...
  const fetchCurrentHierarchy = async () => {
    setIsFetching(true);
    try {
      const result = await authService.getAgencyHierarchy(formData.userId);
      if (result.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await authService.moveAgency({
        userId: formData.userId,
        newMasterAgencyId: formData.newMasterAgencyId
      });

      if (result.success) {
        setSuccess(`Agency "${formData.userId}" moved successfully!`);
        const moved = result.data;
        onMoved && onMoved(moved);
        setFormData({ userId: '', newMasterAgencyId: '' });
        setCurrentHierarchy(null);
      } else {
        setError(result.error || 'Failed to move agency.');
      }
    } catch (err) {
      console.error('Move agency error:', err);
  ...
  return (
    <div className="bg-[#121212] p-6 rounded-xl border border-gray-800">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-2">Agency ID</label>
        ...
        {formData.userId && (
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">New Master Agency</label>
              <div className="relative">
                <Users className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4" />
                <select
                  name="newMasterAgencyId"
                  value={formData.newMasterAgencyId}
                  onChange={handleInputChange}
                  className="w-full pl-10 pr-4 py-3 bg-[#2A2A2A] border border-gray-700 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-[#F72585] focus:ring-1 focus:ring-[#F72585] transition-colors"
                  required
